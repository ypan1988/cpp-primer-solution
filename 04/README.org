...
if (i = 42)   //  ...   
   ...
   It is an error for the condition in the first ~if~ since literals
   are rvalues; The condition in second ~if~ assigns 42 to i and then
   tests the result of the assignment (The condition will be true
   since 42 is not zero).
