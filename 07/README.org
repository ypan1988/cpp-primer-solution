...
** Exercise 7.54
   ...
   No, for a ~constexpr~ function, the only executable statement it can have is
   a return statement.
...
struct Data {
  ...
  std::string s;
   ...
   #+END_SRC
   No. An *aggregate class* whose data members are all of literal type
   is *literal class*. Here ~std::string~ is not a literal type.
