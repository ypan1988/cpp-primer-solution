...
** Exercise 11.11
   ...
   #+BEGIN_SRC C++
bool (*foo)(const Sales_data &lhs, const Sales_data &rhs);
multiset<Sales_data, foo> bookstore(compareIsbn);    
   #+END_SRC
...
bool (*foo)(const Sales_data &lhs, const Sales_data &rhs);
multiset<Sales_data, foo> bookstore(compareIsbn); 
multiset<Sales_data, foo>::iterator mset_it = bookstore.begin();
   #+END_SRC
