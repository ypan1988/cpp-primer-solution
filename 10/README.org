...
** Exercise 10.40
   ...
   + ~copy~ : first and second arguments are Input iterators, last argument is
     an Output iterator.
...
replace_copy_if(beg, end, dest, pred, new_val);   
   ...
   ~replace~ assigns ~new_val~ to all the elements in the range [first,last)
   that compare equal to ~old_val~; ~replace_if~ assigns ~new_val~ to all the
   elements in the range [first,last) for which ~pred~ returns true;
   ~replace_copy~ copies the elements in the range [first,last) to the range
   beginning at result, replacing the appearances of ~old_val~ by ~new_val~;
   ~replace_copy_if~ copies the elements in the range [first,last) to the range
   beginning at result, replacing those for which pred returns true by
   ~new_val~.
