...
** Exercise 13.24
   ...
   ~string~ and ~ps~ in lhs and rhs will point to the same string on the heap.
...
{
    std::cerr << "Message:\t" << contents << std::endl;
...
** Exercise 13.35
   ...
   When we copy a ~Message~, the copy should appear in the same ~Folder~ s as
   the original ~Message~. As a result, we must traverse the ~set~ of ~Folder~
   pointers adding a pointer to the new ~Message~ to each ~Folder~ that points
   to the original ~Message~.

   When a ~Message~ is destroyed, we must remove this ~Message~ from the
   ~Folder~ s that point to it.
...
void Folder::debug_print()
{
    ...
    int ctr = 0;
    for (auto m : msgs) {
        std::cerr << "Message " << ++ctr << ":\t" << m->contents << std::endl;
    }
    std::cerr << std::endl;
}
...
** Exercise 13.38
   ...
   Not efficient. The copy and swap is an elegant way when working with
...
Foo Foo::sorted() const & { return Foo(*this).sorted(); }
   ...
   Okay. ~Foo Foo::sorted() &&~ will be called here.
